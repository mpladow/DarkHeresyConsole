...
namespace Engine.Skills
{
    public class InvestigationSkills:Skills_Base
    {
        ...
        //contstructor 
        public InvestigationSkills(string name, string description, CharacterStat stat, double actioncost = 1, int rank = 0, bool isopposedcheck = false) : base(name, stat, description, isopposedcheck, actioncost)
        {
            TotalSkillModifiers.Add(ReadOnlyLists.GetSkillLevelsById(rank));
        }
        //contstructor 
        public InvestigationSkills(string name, string description, double actioncost = 1, int rank = 0, bool isopposedcheck = false) : base(name, description, isopposedcheck, actioncost)
        {
            TotalSkillModifiers.Add(ReadOnlyLists.GetSkillLevelsById(rank));
        }
